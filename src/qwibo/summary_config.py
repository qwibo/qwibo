...
PROVIDER_ENV_KEYS = {
    "openai": "QWIBO_OPENAI_API_KEY",
    "gemini": "QWIBO_GEMINI_API_KEY",
    "claude": "QWIBO_ANTHROPIC_API_KEY",
    "deepseek": "QWIBO_DEEPSEEK_API_KEY",
    "kimi": "QWIBO_KIMI_API_KEY",
}
...
def load_secrets() -> dict[str, str]:
    ...
    for provider, env_name in PROVIDER_ENV_KEYS.items():
        legacy = env_name.replace("QWIBO_", "QWIBO_")
        val = os.environ.get(env_name, "").strip() or os.environ.get(legacy, "").strip()
        if val:
            data[provider] = val
    return data
