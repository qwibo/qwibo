...
from qwibo.config import SummaryLength, TranscribeConfig
from qwibo.export import export_srt, export_summary_text, export_txt
...
from qwibo.summarize import summarize, summary_skip_reason, unload_summary_models
from qwibo.summarize_providers.registry import provider_label
from qwibo.transcribe import transcribe, unload_model
...
def run_pipeline(job_id: str) -> None:
    ...
    try:
        ...
        if job.summary_requested and result.text.strip():
            ...
            skip_reason = summary_skip_reason(provider_id)
            if skip_reason:
                logger.info("Riassunto saltato per job %s: %s", job_id, skip_reason)
                job.has_summary = False
                job.summary_error = skip_reason
                _progress(job_id, "summarize", 95, "Riassunto saltato (motore non configurato)")
            else:
