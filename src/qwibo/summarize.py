...
def _resolve_provider_id(provider_id: str | None) -> str:
    ...
    from qwibo.summarize_providers.registry import first_available_provider
...
def summary_skip_reason(provider: str | None) -> str | None:
    """Motivo per saltare il riassunto; None se il provider può essere usato."""
    try:
...
def _chunk_length(length: SummaryLength, *, map_reduce_partial: bool = False) -> SummaryLength:
    """Lunghezza per chunk map-reduce: i parziali devono restare brevi."""
    if map_reduce_partial:
    ...
    while len(current) > 1:
        ...
        for bi, batch in enumerate(batches):
            ...
            merge_length = (
                ...
                if len(current) <= batch_size
                else SummaryLength.short
            )
            merged_input = merge_prompt("\n\n".join(batch), merge_length, language)
            next_level.append(
                _complete(
                    impl,
                    merged_input,
...
def _map_reduce_summarize(
    ...
    *,
    ...
    chunk_len = _chunk_length(length, map_reduce_partial=True)
    ...
    for i, chunk in enumerate(chunks):
        ...
        part = _complete(
            impl,
            user_prompt(chunk, chunk_len, language),
            model=used_model,
            on_progress=on_progress,
            language=language,
        )
    ...
    if len(partials) == 1:
        ...
        final_text, strategy = _hierarchical_merge(
            partials, impl, length, used_model, on_progress, language
        )
