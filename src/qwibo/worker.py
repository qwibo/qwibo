...
from qwibo.jobs import claim_next_job, jobs_root, recover_orphaned_running_jobs, reconcile_jobs_with_disk
...
def _pid_running(pid: int) -> bool:
    if sys.platform == "win32":
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            errors="replace",  # tasklist usa la codepage OEM: mai far crashare la decodifica
            check=False,
        )
...
def worker_loop(poll_interval: float = 1.0) -> None:
    ...
    from qwibo.transcribe import warmup_asr
...
def start_background_worker() -> subprocess.Popen[bytes] | None:
    ...
    with _worker_lock:
        ...
        cmd = [sys.executable, "-m", "qwibo.cli", "worker"]
...
def run_worker_forever(poll_interval: float = 1.0) -> None:
    """Avvia il worker nel processo corrente (CLI `qwibo worker`)."""
    _stop_event.clear()
    try:
