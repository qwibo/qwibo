...
"""Interfaccia web Streamlit per Qwibo (deprecata)."""
...
from qwibo.worker import start_background_worker
...
def _summary_tab_message(job: JobRecord, summary_text: str) -> None:
    ...
    if not job.summary_requested:
        st.info("Riassunto disattivato per questo lavoro. Attivalo nella sidebar prima di trascrivere.")
        return
...
def _sidebar_config() -> tuple[TranscribeConfig, bool, str, str]:
    with st.sidebar:
        ...
        st.divider()
        st.caption(f"Qwibo v{__version__} · Python: `{sys.executable}`")
