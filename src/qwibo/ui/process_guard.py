...
from qwibo.config import project_root
...
def find_ui_pids(port: int = 8501) -> set[int]:
    ...
    for pid, cmd in _python_processes():
        if _matches_markers(cmd, UI_APP_MARKERS) or (
            "uvicorn" in cmd.lower() and "qwibo.ui.server" in cmd.lower()
        ):
            pids.add(pid)
    return pids
...
def clear_package_cache() -> None:
    root = project_root() / "src" / "qwibo"
    for cache in root.rglob("__pycache__"):
        shutil.rmtree(cache, ignore_errors=True)


def clear_worker_pid_file() -> None:
    from qwibo.jobs import jobs_root
...
def ensure_clean_ui_environment(port: int = 8501, *, wait_sec: float = 2.0) -> int:
    """Termina UI/worker Qwibo già attivi. Ritorna quanti processi sono stati killati."""
...
def verify_runtime() -> None:
    ...
    import qwibo.config as config_mod

    config_file = Path(config_mod.__file__).resolve()
    expected = (project_root() / "src" / "qwibo" / "config.py").resolve()
    if config_file != expected:
        raise RuntimeError(
            f"Pacchetto qwibo non aggiornato.\n"
    ...
    from qwibo.config import data_dir  # noqa: F401
