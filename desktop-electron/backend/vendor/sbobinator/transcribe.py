...
from qwibo.config import TranscribeConfig, local_model_path
from qwibo.languages import language_label, normalize_language

ProgressCallback = Callable[[str, float, str], None]
from qwibo.export import TranscriptResult, TranscriptSegment
from qwibo.extract import get_duration_sec, prepare_audio, split_audio_chunks
    ...
    with _import_lock:
        ...
        try:
            ...
            raise ImportError(
                ...
                "  pip install -e '.[asr]'\n"
                "oppure usa Docker: docker compose --profile cpu run --rm qwibo"
            ) from exc
...
def _apply_asr_language_hint(model, language: str) -> None:
    """Tenta di impostare un hint lingua sul decoder (best-effort).

    Parakeet v3 rileva spesso la lingua in automatico; non tutte le build NeMo
    espongono un parametro language_id su transcribe().
    """
    ...
    if decoding_cfg is None or not hasattr(decoding_cfg, "decoding"):
        return
    try:
        with open_dict(decoding_cfg.decoding):
            decoding_cfg.decoding.language = lang
        if hasattr(model, "change_decoding_strategy"):
            model.change_decoding_strategy(decoding_cfg.decoding)
            logger.info("Hint lingua ASR applicato: %s", lang)
    except Exception:
        logger.debug("Hint lingua ASR non supportato su questo modello NeMo", exc_info=True)


def _transcribe_file(model, wav_path: Path, *, language: str = "it") -> TranscriptResult:
    lang = normalize_language(language)
    base_kwargs: dict = {
        "batch_size": 1,
    ...
    output = None
    for extra in (
        {"timestamps": True, "language_id": lang},
        {"timestamps": True, "language": lang},
        {"timestamps": True},
        {},
    ):
        try:
            output = model.transcribe([str(wav_path)], **base_kwargs, **extra)
            break
        except TypeError:
            continue
    if output is None:
...
def transcribe(
    ...
    _apply_asr_language_hint(model, lang)

    if on_progress:
        on_progress("extract", 20, "Estrazione audio...")

    lang_label = language_label(lang, short=True)
    ...
    if duration <= config.chunk_threshold_sec:
        ...
        if on_progress:
            on_progress("transcribe", 50, f"Trascrizione in corso ({lang_label}, {duration:.0f} s)...")
        result = _transcribe_file(model, wav_path, language=lang)
        if on_progress:
    ...
    for i, (chunk_path, offset) in enumerate(chunks, start=1):
        ...
        if on_progress:
            pct = 25 + 55 * (i / total)
            on_progress("transcribe", pct, f"Trascrizione chunk {i}/{total} ({lang_label})...")
        results.append((offset, _transcribe_file(model, chunk_path, language=lang)))
