...
**File:** `src/qwibo/ui/server.py`, `templates/partials/job_detail.html` (ex `streamlit_app.deprecated.py` → `_summary_tab_message()`)
...
| **Stato** | ✅ Risolto |
| **Severità** | Alta |
| **Segnalazione** | Avviando `qwiboto2` prima di scaricare `qwiboto1`, il primo sparisce dall'interfaccia |
...
- Ogni qwibota crea una cartella univoca (`YYYYMMDD_HHMMSS`)
...
**File:** `src/qwibo/jobs.py`, `src/qwibo/ui/server.py`
...
|------|-------------------|
| `src/qwibo/ui/app.py` | `_render_queue_panel()` — loop `for job in active` |
| `src/qwibo/ui/app.py` | `_poll_queue_refresh()` — `@st.fragment(run_every=2s)` + `st.rerun(scope="app")` |
| `src/qwibo/jobs.py` | `load_active_queue()`, `enqueue_job()`, `new_job_id()` |
...
**File:** `src/qwibo/ui/server.py`, `templates/partials/queue.html`
...
| **Severità** | Media |
| **Segnalazione** | Dopo "Accoda trascrizione", i file caricati **rimangono visibili** nell'uploader mentre la coda elabora → confusione ("devo ricliccare?" / accodamenti multipli) |
...
**File:** `src/qwibo/ui/server.py`, `templates/index.html`
...
1. Carica N file, clicca "Accoda trascrizione" una volta
...
| `streamlit run` porta 8501 | `uvicorn qwibo.ui.server:app` porta 8501 (stesso URL per l'utente) |
...
| File | Azione |
|------|--------|
| `src/qwibo/ui/server.py` | **Nuovo** — app FastAPI |
| `src/qwibo/ui/templates/` | **Nuovo** — HTML + partials HTMX |
| `src/qwibo/ui/static/style.css` | **Nuovo** — stile (stesso look scuro) |
| `src/qwibo/ui/launch.py` | Avvia uvicorn invece di streamlit; `QWIBO_UI_HOST` per Docker |
| `docker/Dockerfile.*` | `pip install -e ".[ui,summarize]"`, FastAPI |
| `docker/docker-compose.yml` | `QWIBO_UI_HOST=0.0.0.0`, healthcheck HTTP |
| `src/qwibo/ui/streamlit_app.deprecated.py` | **Deprecato** — non più entry point |
...
| **Stato** | ✅ Risolto (v0.3.x — migrazione FastAPI) |
| **Severità** | Alta |
| **Segnalazione** | `cannot import name 'data_dir' from qwibo.config` con path sorgente corretto |
...
**Fix applicato:**  
- Migrazione a **uvicorn** (`qwibo.ui.server:app`) — niente più `streamlit run`
- `process_guard.py`, `restart_ui.py`: kill UI/worker, porta 8501, `verify_runtime()`
- Entry point unico: `qwibo ui` → `launch.py`
...
**Fix:**  
`project_root()` in `config.py` → path assoluto `C:\...\qwibo\models\...`
...
**Sintomo:**  
`cannot import name 'SummaryLength' from qwibo.summarize`
...
`qwibo.exe` può puntare a Python diverso da quello usato in IDE.  
...
`qwibo transcribe video.mp4 -o data/output` sovrascrive `video.txt` se rieseguito.  
...
- Modulo `src/qwibo/http_ssl.py` — `truststore.inject_into_ssl()` se pacchetto installato
...
| **Segnalazione** | Container Qwibo non raggiungibile; porta 8501 usata da `dataunchain-dashboard` |
...
- `src/qwibo/local_llm_download.py` — download automatico se RAM ≥ 16 GB
- Volume Docker `qwibo-qwen` per persistenza
- Comando `qwibo docker-ui` all'avvio container
...
```powershell
docker exec docker-qwibo-cpu-1 sh -c "grep summary_requested /data/output/jobs/*/job.json"
...
| **Segnalazione** | 30/06 — *«bisogna riuscire a trascrivere anche 3 ore di parlato»* (trascrizione + riassunto locale) |
...
5. `python -c "from qwibo.jobs import load_active_queue; ..."` — stesso conteggio DB
...
2. Primo avvio RAM ≥ 16 GB → Qwen scaricato in volume `qwibo-qwen`
...
```powershell
# Verifica path
python -c "from qwibo.config import local_model_path; print(local_model_path())"
...
```powershell
qwibo transcribe data/input/campione-italiano-breve.wav -o data/output
...
| Componente | Path |
|------------|------|
| UI web (FastAPI) | `src/qwibo/ui/server.py` |
| UI deprecata (Streamlit) | `src/qwibo/ui/streamlit_app.deprecated.py` |
| Registro lavori | `src/qwibo/jobs.py` |
| Config / path | `src/qwibo/config.py` |
| Trascrizione | `src/qwibo/transcribe.py` |
| Download modello | `scripts/download-model.ps1` |
| Install locale | `scripts/install-local.ps1` |
| Riassunto LLM | `src/qwibo/summarize.py`, `summarize_providers/` |
| Prompt riassunto | `src/qwibo/summarize_providers/prompt.py` |
| SSL cloud | `src/qwibo/http_ssl.py` |
| Download Qwen | `src/qwibo/local_llm_download.py` |
