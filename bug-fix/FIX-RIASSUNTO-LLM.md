...
La feature **riassunto** in Qwibo **non è accettabile in produzione** nello stato attuale (LexRank + IT5 news). Va **cambiata completamente**.
...
| Path | Ruolo |
|------|--------|
| `src/qwibo/summarize.py` | API pubblica `summarize()`, map-reduce |
| `src/qwibo/summarize_providers/base.py` | `SummaryProvider`, `SummaryRequest`, `SummaryResult` |
| `src/qwibo/summarize_providers/local_qwen.py` | llama.cpp / GGUF |
| `src/qwibo/summarize_providers/openai.py` | OpenAI Chat Completions |
| `src/qwibo/summarize_providers/gemini.py` | Google Generative AI |
| `src/qwibo/summarize_providers/anthropic.py` | Claude Messages API |
| `src/qwibo/summarize_providers/deepseek.py` | DeepSeek (OpenAI-compatible) |
| `src/qwibo/summarize_providers/moonshot.py` | Kimi / Moonshot API |
| `src/qwibo/summary_config.py` | Lettura API key, modello default per provider, path locale |
...
| Dove | Cosa |
|------|------|
| **File locale utente** | `data/.secrets/summary_keys.json` o variabili env `QWIBO_OPENAI_API_KEY`, ecc. |
...
L’utente con **8 GB RAM** può comunque trascrivere e usare **OpenAI/Claude/…** se ha la key.
...
- [x] `qwibo docker-ui`, volume `qwibo-qwen`
...
> «L’utente sceglie se farlo — perde tanto tempo rispetto alla trascrizione.»
...
> **(30/06)** «qwibo è generico, non per wikipedia» (rifiuto glossario dominio)

> **(30/06)** «se c'è un problema sul locale lo devi risolvere — bisogna riuscire a trascrivere anche 3 ore di parlato»
...
| `src/qwibo/summarize_providers/prompt.py` | Prompt (da rafforzare) |
| `src/qwibo/http_ssl.py` | SSL cloud Windows |
| `src/qwibo/local_llm_download.py` | Auto-download Qwen Docker |
